-- Manual Database Setup Script for Skoop
-- Run this in your Supabase SQL Editor if migrations aren't automatically applied
...
DROP POLICY IF EXISTS "Users can only access their own connected accounts" ON public.connected_accounts;
...
DROP TRIGGER IF EXISTS update_content_columns_updated_at ON public.content_columns;
...
-- Uncomment the following lines if you want to add test bookmarks

/*
-- Insert a test bookmark (only if user exists)
...
SELECT 
    ...
    ARRAY['javascript', 'react', 'framework']
FROM public.users 
LIMIT 1
