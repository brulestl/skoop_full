-- T2.2 Test Script: Cross-User Data Isolation (Fixed Version)
-- This script tests that all tables properly prevent cross-user data access
-- Fixed to avoid user_settings trigger issues
...
BEGIN
    ...
    -- Clean up user_settings if it exists (to avoid FK constraint issues)
    BEGIN
        DELETE FROM public.user_settings WHERE user_id IN (test_user_1, test_user_2);
    EXCEPTION
        WHEN undefined_table THEN
            RAISE NOTICE 'user_settings table does not exist, skipping cleanup';
    END;
    
    DELETE FROM public.users WHERE id IN (test_user_1, test_user_2);

    -- Create test users directly in public.users (bypassing auth.users triggers)
    -- This avoids the user_settings trigger issue
...
-- Test 7: Verify RLS policy coverage
SELECT 'Test 7: RLS Policy Coverage' as test_name,
       tablename,
...
-- Clean up user_settings if it exists
DO $$
BEGIN
