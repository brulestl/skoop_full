      ...
      <AnimatePresence>
        ...
        duration: 0.4
            ...
            <div className="flex justify-between items-center px-4 h-14 border-b border-border">
              <Link href="/" className="logo text-2xl font-bold text-primary"><span className="editable-text">SKOOP</span></Link>
            ...
            <nav className="flex flex-col p-6 space-y-6">
              {links.map(link => <Link key={link.href} href={link.href} className="text-xl font-medium hover:text-primary transition-colors" onClick={() => setIsOpen(false)}>
                  {link.label}
                </Link>)}
            </nav>
            ...
            <div className="mt-auto p-6 border-t border-border flex flex-col space-y-4">
              ...
              <Link href="/login" onClick={() => setIsOpen(false)}>
                <Button variant="outline" className="w-full"><span className="editable-text">Log in</span></Button>
              </Link>
              <Link href="/signup" onClick={() => setIsOpen(false)}>
                <Button className="skoop-button-accent w-full"><span className="editable-text">Sign Up Free</span></Button>
              </Link>
            </div>
          </motion.div>}
      </AnimatePresence>
