...
export function SummariesDemo() {
  ...
  return (
    <section className="py-20 lg:py-28" style={{ backgroundColor: '#FCEADE' }}>
      <div className="skoop-container">
        <div className="text-center mb-12">
          <h2 className="section-title-sm mb-4 text-gray-900">Experience AI-Powered Summaries</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-[340px] md:max-w-none mx-auto">
          {demoCards.map((card) => (
            <motion.div
              key={card.id}
              className="skoop-card overflow-hidden flex flex-col h-full"
            ...
            >
              ...
              <div className="p-4 border-b border-border flex items-center">
                ...
                {card.platform === 'reddit' && (
                  <div>
                    ...
                    <div className="flex items-center text-xs text-muted-foreground">
                      <span className="mr-2">u/{card.author}</span>
                      <span>{card.timestamp}</span>
                    </div>
                  </div>
                )}
              ...
              <div className="p-4 flex-1">
                ...
                {card.platform === 'twitter' && card.image && (
                  <div className="mt-3 rounded-md overflow-hidden">
                    <Image 
                      src={card.image} 
                      alt={`Image related to ${card.title}`}
                      width={300}
                      height={200}
              ...
              <div className="border-t border-border p-4 flex justify-between items-center">
                {card.platform === 'twitter' && (
                  <div className="flex text-xs text-muted-foreground">
                    <span className="mr-3">{card.engagement.likes} likes</span>
                    <span>{card.engagement.replies} replies</span>
                  </div>
                )}
                ...
                {card.platform === 'reddit' && (
                  <div className="flex text-xs text-muted-foreground">
                    <span className="mr-3">{card.engagement.upvotes} upvotes</span>
                    <span>{card.engagement.comments} comments</span>
                  </div>
                )}
        ...
        {showSummary && selectedCard && (
          ...
          >
            <motion.div 
              className="w-full max-w-md bg-card h-full overflow-y-auto"
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              exit={{ x: '100%' }}
            ...
            >
              ...
              <div className="p-6">
                <div className="prose prose-sm dark:prose-invert">
                  ...
                  <ul>
                    <li>Content discusses advancements in the field of artificial intelligence and its applications</li>
                    <li>Research findings highlight practical implementations in modern software development</li>
                    <li>Author provides evidence-based analysis of current technology trends</li>
                    <li>The post received significant engagement from the community</li>
                  </ul>
                  <p>
