  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button className="skoop-button-primary">Connect Account</Button>
      </div>
      
      {/* User info */}
      <div className="skoop-card p-6 mb-8">
        <div className="flex items-center">
          <div className="w-16 h-16 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-xl mr-4">
            JD
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6 pt-6 border-t border-border">
          <div>
            ...
            <div className="font-semibold text-2xl">299</div>
          </div>
          <div>
            <div className="text-sm text-muted-foreground">Collections</div>
            <div className="font-semibold text-2xl">15</div>
          </div>
          <div>
            <div className="text-sm text-muted-foreground">Member Since</div>
            <div className="font-semibold">March, 2023</div>
          </div>
        </div>
      </div>
...
function ConnectedAccountCard({ account }: ConnectedAccountCardProps) {
  const [expanded, setExpanded] = useState(false);
  const Icon = account.icon;
  
  return (
    <motion.div 
      className="skoop-card overflow-hidden"
      ...
      <div className="p-4 flex justify-between items-center">
        <div className="flex items-center">
          <div className={cn(
            ...
            account.status === "connected" ? "bg-primary/10" : "bg-destructive/10"
          )}>
            <Icon className={cn(
              "h-5 w-5",
              account.status === "connected" ? "text-primary" : "text-destructive"
            )} />
          </div>
          <div>
            <div className="flex items-center">
        ...
        <div className="bg-muted/30 p-4 border-t border-border">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="text-sm">{account.itemCount} saved items</div>
            </div>
          </div>
