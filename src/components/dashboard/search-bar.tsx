  ...
  return <>
      <div className={cn("relative", minimal ? "w-full" : "max-w-2xl mx-auto w-full")}>
        <div className={cn("flex items-center rounded-lg border border-input bg-background px-3 py-2 text-sm transition-all", focused ? "border-primary ring-2 ring-primary ring-opacity-30" : "", minimal ? "" : "shadow-sm")}>
          ...
          <input ref={inputRef} type="text" placeholder={placeholder} className="flex-1 bg-transparent outline-none placeholder:text-muted-foreground" value={query} onChange={e => setQuery(e.target.value)} onFocus={handleInputFocus} onBlur={() => setFocused(false)} />
          
          {query && <Button variant="ghost" size="icon" className="h-5 w-5 text-muted-foreground hover:text-foreground" onClick={() => setQuery("")}>
              <X className="h-3 w-3" />
              <span className="sr-only"><span className="editable-text">Clear search</span></span>
            </Button>}
        </div>
        
        {!minimal && <div className="absolute right-3 top-2 text-xs text-muted-foreground pointer-events-none">
            <kbd className="px-1.5 py-0.5 rounded border border-border bg-muted font-mono"><span className="editable-text">
              ⌘K
            </span></kbd>
          </div>}
      </div>
              ...
              stiffness: 300
                ...
                <div className="p-4 border-b border-border">
                  <div className="flex items-center">
                    ...
                    <input autoFocus type="text" placeholder="Search by meaning, not just keywords..." className="flex-1 bg-transparent outline-none text-base" value={query} onChange={e => setQuery(e.target.value)} />
                    {query && <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground" onClick={() => setQuery("")}>
                        <X className="h-4 w-4" />
                        <span className="sr-only"><span className="editable-text">Clear search</span></span>
                      </Button>}
                  </div>
                  ...
                  <div className="mt-2 flex justify-between items-center">
                    <div className="text-xs text-muted-foreground"><span className="editable-text">
                      Semantic Search powered by </span>{selectedModel === 'claude-bedrock' ? 'Claude' : 'OpenAI'}
                    </div>
                    <Button variant="outline" size="sm" className="text-xs h-7" onClick={toggleModel}><span className="editable-text">
                ...
                <div className="max-h-[60vh] overflow-y-auto">
                  {loading ? <div className="py-8 flex justify-center items-center">
                      <Loader2 className="h-6 w-6 text-primary animate-spin" />
                      <span className="ml-2 text-sm text-muted-foreground"><span className="editable-text">Searching with AI...</span></span>
                    </div> : results.length > 0 ? <div className="divide-y divide-border">
                    ...
                    setShowModal(false);
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {result.description && <p className="text-sm text-muted-foreground line-clamp-2">
                                  {result.description}
                                </p>}
                            </div>
                            <div className="ml-2">
                    ...
                    </div> : query ? <div className="py-8 px-4 text-center text-muted-foreground">
                      <p><span className="editable-text">No results found for "</span>{query}<span className="editable-text">"</span></p>
                      <p className="text-sm mt-1"><span className="editable-text">Try using different keywords or browse your collections</span></p>
                    </div> : <div className="py-8 px-4 text-center text-muted-foreground">
                      <p><span className="editable-text">Start typing to search semantically across all saved content</span></p>
                      <p className="text-sm mt-1"><span className="editable-text">Try "GraphQL auth pattern" or "React optimization"</span></p>
                    </div>}
                </div>
                
                <div className="px-4 py-3 bg-muted/30 border-t border-border text-xs text-muted-foreground flex justify-between">
                  <div><span className="editable-text">
                    Press </span><kbd className="px-1 py-0.5 rounded border border-border bg-muted font-mono"><span className="editable-text">ESC</span></kbd><span className="editable-text"> to close
                  </span></div>
                  <div>
